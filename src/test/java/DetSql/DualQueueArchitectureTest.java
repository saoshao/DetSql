...
public class DualQueueArchitectureTest {
    ...
    void testRejectionPolicy() throws Exception {
        ...
        // P0-2 修复后：RECEIVE_EXECUTOR 仍使用 CallerRunsPolicy
        ...
        // P0-2 修复后：SCAN_EXECUTOR 使用自定义拒绝处理器，不再使用 CallerRunsPolicy
        // 自定义处理器会记录警告并丢弃任务，避免 UI 冻结
        assertFalse(scanExecutor.getRejectedExecutionHandler() 
            instanceof ThreadPoolExecutor.CallerRunsPolicy,
            "SCAN_EXECUTOR 不应该使用 CallerRunsPolicy（P0-2 修复：避免 UI 冻结）");
        
        // 验证自定义拒绝处理器不会是 null
        assertNotNull(scanExecutor.getRejectedExecutionHandler(),
            "SCAN_EXECUTOR 应该有自定义的拒绝处理器");
    }
