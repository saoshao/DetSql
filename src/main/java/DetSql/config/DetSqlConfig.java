...
public class DetSqlConfig {
     ...
     * SQL 错误检测规则列表
     */
    private String[] errorDetectionRules = DefaultConfig.ERROR_DETECTION_RULES.clone();
    ...
    public void load(String configPath) throws IOException {
        ...
        // 加载错误检测规则
        String errorRules = prop.getProperty("errorDetectionRules", "").trim();
        if (errorRules.isEmpty()) {
            this.errorDetectionRules = DefaultConfig.ERROR_DETECTION_RULES.clone();
        } else {
            this.errorDetectionRules = errorRules.split("\\|");
        }
    }
    ...
    public void save(String configPath) throws IOException {
        ...
        prop.setProperty("errorDetectionRules", String.join("|", errorDetectionRules));
    ...
    public String[] getErrorDetectionRules() {
        return errorDetectionRules;
    }

    public void setErrorDetectionRules(String[] errorDetectionRules) {
        var old = this.errorDetectionRules;
        this.errorDetectionRules = errorDetectionRules;
        fireChange("errorDetectionRules", old, errorDetectionRules);
    }
